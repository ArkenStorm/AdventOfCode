...
# def get_possible_dirs(pos: tuple, curr_dr: tuple, streak: int, path: set):
# 	bounded_dirs = list(filter(
# 		lambda d: 0 <= pos[0] + d[0] < len(grid) and 
# 			0 <= pos[1] + d[1] < len(grid[0]) and 
# 			d != inverses[curr_dr] and 
# 			((pos[0] + d[0], pos[1] + d[1]), curr_dr) not in path,
# 		dirs
# 	))
# 	return bounded_dirs if streak < 3 else list(filter(lambda d: d != curr_dr, bounded_dirs))
...
def dijkstra(q: list, min_streak=1, max_streak=3):
	...
	while len(q) > 0:
		...
		# possibles = get_possible_dirs(pos, dir, streak, path)
		# for new_dir in possibles:
